...
//		}
	...
	function ResolveFilenameToAbsolute($filename) {
		...
		if (!$this->config_allow_src_above_docroot && !preg_match('@^'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', realpath($this->config_document_root)).'@i'), $AbsoluteFilename)) {
		...
		if (!$this->config_allow_src_above_phpthumb && !preg_match('@^'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__)).'@i'), $AbsoluteFilename)) {
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				if (preg_match('/Version: ImageMagick ([0-9])/', $versionstring[1], $matches)) {
